...
if not statusd_netmon_eth then
  statusd_netmon_eth = {
      device = "eth0",
...
local settings
if statusd ~= nil then
    settings = table.join(statusd.get_config("netmon_eth"), statusd_netmon_eth)
end
...
--
local function parse_netmon_eth_info()
    local s
    local lin, lout
...
--
local function update_netmon_eth_info()
    local s
    local lin, lout
    ...
    lin, lout = parse_netmon_eth_info()
    if not lin or not lout then
	-- you should never reach here
        statusd.inform("netmon_eth", "oops")
	statusd.inform("netmon_eth_hint", "critical")
        return
    end
	...
	statusd.inform("netmon_eth_avgin", fmt(avgin))
	statusd.inform("netmon_eth_avgin_hint",
		       get_hint("avgin", avgin))
	statusd.inform("netmon_eth_avgout", fmt(avgout))
	statusd.inform("netmon_eth_avgout_hint",
	...
	if count then
	    ...
	    statusd.inform("netmon_eth_count", tostring(count))
	    statusd.inform("netmon_eth_count_hint",
			   get_hint("count", count))
	else
	    output = "[?] " .. output
	    statusd.inform("netmon_eth_count", "?")
	    statusd.inform("netmon_eth_count_hint", "critical")
	end
    end

    statusd.inform("netmon_eth_kbsin", fmt(kbsin))
    statusd.inform("netmon_eth_kbsin_hint",
		   get_hint("kbsin", kbsin))
    statusd.inform("netmon_eth_kbsout", fmt(kbsout))
    statusd.inform("netmon_eth_kbsout_hint",
		   get_hint("kbsout", kbsout))
    statusd.inform("netmon_eth", output)

    timer:set(settings.interval, update_netmon_eth_info)
end
...
-- start the timer
-- 
local function init_netmon_eth_monitor()
    if statusd ~= nil then
    if sanity_check() then
        timer = statusd.create_timer()
        last[0], last[1] = parse_netmon_eth_info()
        ...
        statusd.inform("netmon_eth_template", "xxxxxxxxxxxxxxxxxxxxxxx")
        update_netmon_eth_info()
    else
        statusd.inform("netmon_eth", "oops")
        statusd.inform("netmon_eth_hint", "critical")
    end
