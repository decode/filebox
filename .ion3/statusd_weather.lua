...
local function parse_weather(s)
   ...
   _, _, w.tempF, w.tempC = string.find(s, "Temperature:%s(-?%d+)%sF%s%((-?%d+)%sC%)%c" )
   _, _, w.dewpointF, w.dewpointC = string.find(s, "Dew%sPoint:%s(-?%d+)%sF%s%((-?%d+)%sC%)" )
   _, _, w.windchillF, w.windchillC = string.find(s, "Windchill:%s(-?%d+)%sF%s%((-?%d+)%sC%)" )
   _, _, w.humidity = string.find(s, "Relative%sHumidity:%s(%d+)%%")
   _, _, w.pressure = string.find(s, "Pressure%s.+%((.+)%shPa%)" )
