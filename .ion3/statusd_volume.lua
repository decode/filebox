...
local function get_volume()
   local f=io.popen('amixer cget numid=1', 'r')
   local s=f:read('*all')
   f:close()

   local _, _, master_level = string.find(s, "values=(%d+),%d+")

   f=io.popen('amixer cget numid=2', 'r')
   s=f:read('*all')
   f:close()

   local _, _, master_state = string.find(s, "values=(%a*),%a*")

   master_level = math.ceil((master_level/39)*100)
