...
local defaults = {
	...
	turn_off = 50,		     --> Turn Off the alarm if it annoys you too much time: 
				     --  (turn_off*flash_interval) milliseconds.
				     --> If you want permanent alarms: 3*999*999 is OK.
}
local settings
if statusd ~= nil then
	settings = table.join(statusd.get_config("flashing"), defaults)
end
...
if settings ~= nil then
	message_length = string.len(settings.alarm_message)
	turn_off = settings.turn_off
